...
// STEP 1: Define your data structures
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (scoreLabels[i] == prediction.PredictedLabels)
                {
                    score = (decimal) prediction.Score[i];
                    break;
                }
            }

            Console.WriteLine($"Predicted flower type is: {prediction.PredictedLabels} - {score}");

            for (int i = 0; i < scoreLabels.Length; i++)
            {
                Console.WriteLine($"Score of {scoreLabels[i]} is {(decimal)prediction.Score[i]}");
            }
        }
    }
}